...
export const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  return (
    <div className="flex h-full flex-col relative overflow-hidden bg-gray-900">
        ...
        {pinnedMessage && (
            <div className={`absolute top-20 left-4 right-4 z-20 flex items-center p-2 rounded-xl shadow-lg cursor-pointer animate-slide-up ${appTheme === 'pastel' ? 'bg-white/90 text-gray-800' : 'bg-gray-800/90 text-white backdrop-blur-md border border-white/10'}`}>
        ...
        <div className={`shrink-0 z-30 px-6 py-3 flex justify-between items-center transition-all duration-300 ${getHeaderClass()} m-4 rounded-[24px]`}>
            <div className="flex items-center space-x-4">
                <button onClick={onBack} className={`md:hidden p-2 rounded-full transition ${isPastel ? 'hover:bg-gray-100' : 'hover:bg-white/10 text-white'}`}>
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>
                <div className="relative cursor-pointer group" onClick={() => onViewImage(partner.avatar)}>
                ...
                <div className="flex flex-col">
                    <h2 className={`text-xl font-bold leading-none ${isPastel ? 'text-gray-900' : 'text-white'}`}>{partner.name}</h2>
            ...
            <div className="flex space-x-1">
                <button onClick={() => onStartCall(CallType.AUDIO)} className={`p-3 rounded-full transition ${isPastel ? 'hover:bg-gray-100 text-gray-600' : 'hover:bg-white/10 text-white'}`}>
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                </button>
                <button onClick={() => onStartCall(CallType.VIDEO)} className={`p-3 rounded-full transition ${isPastel ? 'hover:bg-gray-100 text-gray-600' : 'hover:bg-white/10 text-white'}`}>
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                </button>
                <button className={`p-3 rounded-full transition ${isPastel ? 'hover:bg-gray-100 text-gray-600' : 'hover:bg-white/10 text-white'}`}>
        ...
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto p-4 z-10 scrollbar-hide space-y-4 pt-20" onClick={() => { setShowAttachMenu(false); setHoveredMessageId(null); }}>
                ...
                return (
                    ...
                    >
                        ...
                        <div className={`max-w-[85%] md:max-w-[65%] relative transition-transform duration-200 ${isHovered ? 'scale-[1.01]' : ''}`}>
                            {/* Reply Context */}
                            {msg.replyToId && (
                                <div className={`text-xs mb-1 px-3 py-1 rounded-lg opacity-80 border-l-2 border-white/50 ${isMe ? 'text-right' : 'text-left'}`}>
                            ...
                            <div className={`px-5 py-3 ${getBubbleClass(isMe)} rounded-[26px] ${isMe ? (showTail ? 'rounded-tr-sm' : '') : (showTail ? 'rounded-tl-sm' : '')} relative`}>
                                            ...
                                            {msg.pollOptions.map(opt => {
                                                ...
                                                return (
                                                    <div key={opt.id} onClick={() => onVotePoll(msg.id, opt.id)} className="cursor-pointer">
                                                        ...
                                                        {opt.votes.length > 0 && <div className="flex -space-x-1 mt-1">{opt.votes.slice(0,3).map((v, i) => <div key={i} className="w-4 h-4 rounded-full bg-gray-300 border border-black/10"></div>)}</div>}
                                                    </div>
                                                );
                                            })}
                                        </div>
                                ...
                                {(msg.type === MessageType.TEXT || !msg.type) && (
                                    <p className="text-[15.5px] leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                                )}
                            ...
                            {msg.reactions.length > 0 && (
                                <div className={`absolute -bottom-3 ${isMe ? 'right-4' : 'left-4'} flex items-center -space-x-1 bg-white dark:bg-gray-800 rounded-full px-2 py-0.5 shadow-md border border-gray-100 dark:border-gray-700`}>
                                    ...
                                    {msg.reactions.reduce((a, b) => a + b.count, 0) > 1 && (
                                        <span className="text-[10px] font-bold text-gray-500 pl-1">{msg.reactions.reduce((a, b) => a + b.count, 0)}</span>
                                    )}
                                </div>
                            )}
                        </div>
        ...
        {replyingTo && (
            <div className={`mx-4 mb-2 p-3 rounded-xl flex justify-between items-center animate-slide-up ${isPastel ? 'bg-white shadow-lg' : 'bg-gray-800/90 border border-gray-700'}`}>
                <div className="flex-1 border-l-4 border-emerald-500 pl-3">
                    <p className={`text-xs font-bold ${isPastel ? 'text-emerald-600' : 'text-emerald-400'}`}>Replying to {replyingTo.senderId === currentUser.id ? 'yourself' : partner.name}</p>
        ...
        <div className="p-4 z-20 shrink-0">
            ...
            <div className={`flex items-end space-x-2 p-1.5 rounded-[30px] shadow-2xl transition-all ${getInputClass()}`}>
                ...
                <div className="pb-1 pr-1">
                     {inputValue.trim() ? (
                        <button 
                            ...
                            className={`p-3 rounded-full text-white shadow-lg transform transition active:scale-95 bg-gradient-to-r from-emerald-500 to-cyan-500`}
                        >
                            {editingMessageId ? 
