...
export const Sidebar: React.FC<SidebarProps> = ({ 
          ...
          <div className="animate-pop-in px-4 pb-4">
            ...
            <div className="bg-white dark:bg-dark-hover rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 space-y-4">
               <div className="space-y-4">
                 ...
                 <div className="flex flex-col">
                   <span className="text-gray-800 dark:text-gray-200 font-medium mb-1">Profile Photo</span>
                   <select className="bg-gray-50 dark:bg-dark-input rounded-lg p-2.5 text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-nexus-500 outline-none">
                     <option>Everyone</option>
                     <option>My Contacts</option>
                     <option>Nobody</option>
               ...
               <Toggle label="Read Receipts" checked={true} onChange={() => {}} />
               <div className="pt-2">
                 <button className="text-red-500 text-sm font-semibold hover:bg-red-50 dark:hover:bg-red-900/20 px-3 py-2 rounded-lg transition w-full text-left">Blocked Contacts (3)</button>
               </div>
            </div>
          </div>
        );
            ...
            <div className="bg-white dark:bg-dark-hover rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 space-y-6">
                 <div className="flex items-center justify-between">
                    ...
                    <button onClick={onToggleTheme} className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-dark-input px-4 py-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition ring-1 ring-gray-200 dark:ring-gray-700">
                       {currentTheme === 'light' ? 
                         <><span className="font-semibold">Light Mode</span><div className="w-3 h-3 bg-yellow-400 rounded-full shadow-sm ring-2 ring-white"></div></> : 
                         <><span className="font-semibold">Dark Mode</span><div className="w-3 h-3 bg-indigo-500 rounded-full shadow-sm ring-2 ring-indigo-300"></div></>}
                    </button>
                 </div>
                 <div>
                    <span className="text-gray-800 dark:text-gray-200 block mb-3 font-medium">Wallpaper</span>
                    <div className="flex space-x-3 overflow-x-auto pb-4 scrollbar-hide">
                       {['bg-gray-100', 'bg-blue-100', 'bg-rose-100', 'bg-emerald-100', 'bg-violet-100'].map((color, i) => (
                         <div key={i} className={`w-16 h-24 rounded-xl ${color} border-2 border-transparent hover:border-nexus-500 cursor-pointer shadow-sm transition-all transform hover:scale-105 active:scale-95`}></div>
                       ))}
                    </div>
                 </div>
                 <div className="pt-2 space-y-2">
                    <button className="flex items-center space-x-3 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition w-full py-3 px-4 rounded-xl">
                       ...
                       <span className="font-medium">Chat Backup</span>
                    </button>
                    <button className="flex items-center space-x-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition w-full py-3 px-4 rounded-xl">
      ...
      case 'notifications': 
        return (
          <div className="animate-pop-in px-4 pb-4">
            ...
            <div className="bg-white dark:bg-dark-hover rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 space-y-1 divide-y divide-gray-100 dark:divide-gray-700">
               <Toggle label="Conversation Tones" checked={true} onChange={() => {}} />
               <Toggle label="Desktop Notifications" checked={false} onChange={() => {}} />
               <Toggle label="Show Message Previews" checked={true} onChange={() => {}} />
               <Toggle label="Reaction Notifications" checked={true} onChange={() => {}} />
            </div>
          </div>
        );
      case 'storage': 
        return (
          <div className="animate-pop-in px-4 pb-4">
            ...
            <div className="bg-white dark:bg-dark-hover rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 space-y-6">
                <div className="p-4 bg-gray-50 dark:bg-dark-input rounded-xl border border-gray-100 dark:border-gray-700">
                   <div className="flex justify-between items-end mb-3">
                      <span className="text-gray-500 dark:text-gray-400 text-sm font-medium">Storage Used</span>
                      <span className="text-gray-900 dark:text-white font-bold text-lg">2.4 GB</span>
                   </div>
                   <div className="w-full bg-gray-200 dark:bg-gray-700 h-2.5 rounded-full overflow-hidden">
                      <div className="bg-nexus-500 h-full w-[45%] rounded-full shadow-glow"></div>
                   </div>
                </div>
                
                <div>
                   <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-4">Media Auto-Download</h3>
                   <div className="space-y-4">
                       {['Photos', 'Audio', 'Videos', 'Documents'].map(type => (
                           <label key={type} className="flex items-center justify-between cursor-pointer group">
                              ...
                              <input type="checkbox" className="w-5 h-5 text-nexus-600 rounded bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:ring-nexus-500 transition" defaultChecked={type !== 'Videos'} />
                           </label>
                       ))}
                   </div>
                </div>
      ...
      case 'help': 
        return (
          <div className="animate-pop-in px-4 pb-4">
            ...
            <div className="bg-white dark:bg-dark-hover rounded-2xl p-2 shadow-sm border border-gray-100 dark:border-gray-700/50 space-y-1">
               {['Help Center', 'Contact Us', 'Terms and Privacy Policy', 'App Info'].map(item => (
                 <button key={item} className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition group">
                    <span className="text-gray-800 dark:text-gray-200 font-medium group-hover:text-nexus-600 dark:group-hover:text-nexus-400 transition-colors">{item}</span>
                    <svg className="w-5 h-5 text-gray-400 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                 </button>
               ))}
            </div>
          </div>
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-dark-bg border-r border-gray-200 dark:border-gray-700 w-full md:w-[400px] transition-colors duration-300">
      {/* Header */}
         ...
         <div className="flex bg-gray-200/60 dark:bg-gray-800/60 p-1.5 rounded-xl">
            {[
              ...
              <button 
                ...
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${activeTab === tab.id ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
              >
                {tab.label}
              </button>
            ))}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide px-3 pb-4">
        ...
        {activeTab === 'chats' && (
          <>
            <div className="px-2 mb-4 sticky top-0 z-10 pt-2">
        ...
        {/* SETTINGS TAB CONTENT RENDERED ABOVE */}
